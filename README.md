# 👋 Hi, I'm Sandeep Soma

### 💻 SDET | QA Engineer | Aspiring Devops Engineer | 3 Years of Experience

     Automation | Devops | Cloud | GEN AI

I’m a passionate Software Engineer driven by **curiosity**, **integrity**, and **a mindset of continuous improvement**.

I enjoy transforming ideas into **reliable**, **automated systems that enhance both speed and quality**.
With a strong focus on **test architecture**, **CI/CD integration**, and **scalable frameworks**, I strive to make quality a seamless part of the development lifecycle — not an afterthought.

As I continue to grow, I’m expanding my expertise toward DevOps and platform automation, aiming to build systems that are efficient, maintainable, and built for long-term impact.

---

## Professional Skills

**Languages:**  
Java | Python | Javascript  

**Cloud Platforms:**  
GCP | Azure  

**Automation:**  
Selenium | TestNG | Cucumber | Perfecto | Sauce labs

**API Testing:**  
Rest Assured | Postman

**Version Control, CI/CD & DevOps:**  
GIT | Jenkins | GitHub Actions

**Reporting Tools:**  
Extent Report 

**Databases:**  
MySQL  

**Test Management:**  
Jira | Qtest   

---

## 🔭 Currently Exploring

- 🧠 **Generative AI & Agentic AI** – exploring how AI can enhance automation, quality insights, and productivity
- ⚙️ **DevOps Path** – deepening knowledge in CI/CD, cloud, and infrastructure automation

---

### ✍️ Read My Insights

I occasionally share my learnings and automations tips here:  👉 [![Medium](https://img.shields.io/badge/Medium-12100E?logo=medium&logoColor=white)](https://medium.com/@sandeep031)

---

### 📫 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?logo=linkedin&logoColor=white)](https://linkedin.com/in/sandeep031)    [![GitHub](https://img.shields.io/badge/GitHub-black?logo=github&logoColor=white)](https://github.com/Sandeepsoma31)

---
